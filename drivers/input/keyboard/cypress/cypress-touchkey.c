...
static void led_fadeout_process(struct work_struct *work)
{
	...
	if (!led_abort_fade) {
		...
		i2c_touchkey_write(bl_tkey_i2c, (u8 *) &touchkey_led_status, 1);
	}
				 ...
				 size_t size)
{
	...
	if (!led_on_keypress && led_fadein && data > 1 && touchkey_enabled_flag) {
		/* Break-off running fade-out process */
		led_abort_fade = 1;
		cancel_work_sync(&led_fadeout_work);
		led_abort_fade = 0;
	...
	if (led_fadeout && data == 0 && touchkey_enabled_flag) {
		led_abort_fade = 1;
	...
	if (!led_on_keypress && data > 1 && touchkey_enabled_flag) {
